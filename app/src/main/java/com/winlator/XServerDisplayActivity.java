...
import com.winlator.gamehubpp.Resolver;
...
public class XServerDisplayActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!isGenerateWineprefix()) {
            ...
            if (shortcut != null) {
                String gameId = "c" + container.id + "_" + shortcut.file.getName();
                LaunchCoordinator.get(this).prepareLaunch(gameId);
            }
    ...
    private void exit() {
        if (shortcut != null) LaunchCoordinator.get(this).onLaunchAborted();
        winHandler.stop();
        if (environment != null) environment.stopEnvironmentComponents();
    ...
    private void setupXEnvironment() {
        ...
        guestProgramLauncherComponent.setTerminationCallback((status) -> {
            if (shortcut != null) LaunchCoordinator.get(XServerDisplayActivity.this).onGameExit(status);
            exit();
        });
